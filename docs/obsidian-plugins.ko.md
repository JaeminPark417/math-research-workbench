...
커뮤니티 플러그인은 사용자를 대신해 실행되고 vault에 접근할 수 있는 추가
코드이며, 대부분은 제3자 프로젝트입니다. 해결하려는 문제를 한 문장으로 분명히 말할 수 있을 때만 설치하세요.
...
## Workbench가 제공하는 수식 구분자 호환 플러그인

**MRW LaTeX Delimiter Compatibility**는 이 workbench에 함께 들어 있는 유일한 플러그인입니다. 기존 문서나 가져온 Markdown에서 `\(...\)`와 `\[...\]`를 표시하고, 표 안의 수식도 안정적으로 렌더링하도록 돕습니다. 일반적인 `$...$`와 `$$...$$` 표기는 이 플러그인 없이도 작동하므로 설치는 선택 사항입니다.

플러그인 원본은 사용자가 설치에 동의하기 전까지 `optional/` 아래에서 실행되지 않습니다. 이 프로젝트가 작성해 MIT 라이선스로 제공하지만 Obsidian 공식 목록에는 등록되어 있지 않습니다. **Browse**에서 이 ID를 검색하지 마세요. 현재 코드는 네트워크 요청, 사용 정보 수집, 연구 노트 쓰기를 하지 않습니다. 다만 커뮤니티 플러그인을 활성화하면 그 코드는 vault에 플러그인 수준으로 접근하며, Obsidian의 전체 커뮤니티 플러그인 신뢰 설정도 켜야 합니다.

초기 설정에서 Codex가 이 내용을 설명하고 별도 동의를 받은 뒤에만 고정 경로 설치 도우미를 실행합니다. 설치 도우미는 검토된 `main.js`와 `manifest.json`만 복사하고, `community-plugins.json`을 수정하거나 플러그인을 활성화하지 않습니다. 사용자가 설치된 플러그인 목록에서 **MRW LaTeX Delimiter Compatibility**를 직접 켠 뒤 Reading view, Live Preview, 표 셀에서 커서를 옮길 때의 수식, Obsidian 재시작 후 동작을 확인합니다.

이 플러그인은 Obsidian의 **Check for updates**로 갱신되지 않고 Workbench 릴리스와 함께 갱신됩니다. 설치본을 갱신하기 전에는 [안전한 업데이트](updating.ko.md)를 읽으세요.

## 공식 목록 플러그인의 안전한 설치 순서
...
Codex가 이 화면들을 차례로 안내할 수는 있지만, 제3자 플러그인 코드를 직접 내려받거나 복사해서는 안 됩니다. 위의 고정된 Workbench 번들만 별도 동의 후 포함된 설치 도우미로 복사할 수 있습니다. 업데이트도 한 번에 하나씩 검토하세요. 커뮤니티 플러그인이 항상 자동으로 업데이트되는 것은 아닙니다.
...
| 가져온 `\(...\)` 또는 `\[...\]` 수식과 표 안 수식 표시 | 번들된 MRW 호환 플러그인 고려 |
