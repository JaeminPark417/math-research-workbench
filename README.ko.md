...
- 번들된 `$pro-context-bundle` 상담 스킬에서 사용할 앱 내 Browser의 ChatGPT
  로그인 여부(해당 Browser를 사용할 수 있는 경우)
...
## 프로젝트의 연구 상태를 정리하는 방식

평소처럼 Codex와 대화하며 질문하고, 정보를 추가하고, 여러 접근을 시험하면서 문제를 풀면 됩니다. Codex는 각 활성 프로젝트의 `README.md`를 간결한 **Research State Spine(연구 상태 중심축)**으로 관리하여 중요한 수학적 상태를 언제든 다시 파악할 수 있게 합니다.

중심축에는 논문화나 연구 결정에 필요한 안정된 대상을 명시적인 식별자로 기록합니다. 정의는 `Def-NNN`, 보조정리는 `Lem-NNN`, 명제는 `Prop-NNN`, 정리는 `Thm-NNN`, 따름정리는 `Cor-NNN`, 해결되지 않은 빈틈은 `Gap-NNN`으로 표시합니다. 이 식별자는 연구를 정리하기 위한 것이며 주장이 참이라고 인증하지 않습니다. 어떤 결과가 논문이나 이후 판단에서 계속 사용할 안정된 하위 귀결이 되었을 때만 `Cor-NNN`을 부여합니다.

검색과 자동 검사를 안정적으로 유지하기 위해 중심축 내부의 구조 제목과 표 머리글은 서식에 적힌 영문을 그대로 사용합니다. 설명과 일반 제목은 선택한 언어로 작성하며, 이 표는 Codex가 관리하므로 사용자가 영문 표 구조를 직접 편집할 필요는 없습니다.

탐색 계산, 임시 관찰, 실패한 접근, 아직 불확실한 귀결은 `sessions/`, `logs/`, `drafts/`에 남깁니다. Codex는 논문화나 연구 결정에 필요한 안정된 대상만 중심축으로 올리므로 사용자가 표를 직접 편집할 필요는 없습니다.

예를 들어 평소 말하듯 다음과 같이 요청할 수 있습니다.

```text
이 문제를 지금처럼 나와 함께 계속 연구해줘. 논문화에 필요한 안정된 정의, 보조정리, 명제, 정리, 따름정리, 해결되지 않은 빈틈이 생기면 프로젝트 Research State Spine을 갱신하고, 탐색 중인 계산과 임시 생각은 logs나 drafts에 남겨줘.
```
...
- 로그인했다고 연구자료 전송까지 허용한 것은 아닙니다. Claude 검토, Browser 파일
  ...
  보여주고 그 한 번의 작업에 대해 다시 승인을 받아야 합니다. 번들된
  `$pro-context-bundle`은 Pro 답변을 시간 제한 없이 기다리며, 상담 도중 사용자가
  명시적으로 요청하지 않는 한 `Answer now`를 누르지 않습니다.
- 비밀번호, passkey, MFA 응답, OAuth 코드는 사용자가 직접 입력합니다. Codex는
  인증 화면에서 멈추고 그 화면을 검사하거나 캡처하지 않습니다.
- 프로젝트가 제공하는 LaTeX 구분자 호환 플러그인은 실행되지 않는 상태로 함께 들어 있으며, 별도 동의·사용자 직접 활성화·수식 렌더링 테스트를 거쳐야 합니다. 제3자 커뮤니티 확장 기능은 계속 선택 사항이므로 [확장 기능 안내](docs/obsidian-plugins.ko.md)를 읽고 필요한 것만 하나씩 설치합니다.
...
수 있습니다. 번들된 `$pro-context-bundle`이 이 기능을 사용합니다. 해당 스킬을
쓸 계획이 없다면 `나중에`를 선택하세요. 공식
